...
Uses the tree-sitter parse tree of the node's `source_code` to identify and replace:

| AST Node Type | Action |
|---|---|
| `jsx_element`, `jsx_self_closing_element` | Replace with `/* [JSX: N elements] */` |
| `array` with > 3 elements | Replace with `/* [array: N items] */` |
| `object` with > 4 properties | Replace with `/* [object: N props] */` |
| `template_string` with > 100 chars | Replace with `` `/* [template: N chars] */` `` |
| `comment` block | Remove entirely |
| `string_literal` > 80 chars | Replace with `"/* [string: N chars] */"` |
| `import_declaration` | Keep verbatim (critical for dependency analysis) |
| `call_expression` | Keep verbatim (external calls preserved) |
| `if_statement`, `switch_statement` | Keep condition + fold branches if body > 10 lines |
